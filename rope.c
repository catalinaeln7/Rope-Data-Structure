...
// Functie de concatenare pe noduri
RopeNode* __concat(RopeNode* node1, RopeNode* node2)
{
	...
	// Root-ul cantareste cat tot arborele stang
	root->weight = getTotalWeight(node1);

	return root;
}

// Functia principala de concatenare
...
// Functie de index cand se da doar nodul root
char __indexRope(RopeNode* root, int idx)
{
	// Daca am ajuns la frunza, returnez al idx-lea caracter
	if(strcmp(root->str, EMPTY))
	{
		return root->str[idx];
	}

	// idx este din subarborele stang
	if(idx < root->weight)
	{
		return __indexRope(root->left, idx);
	}

	// idx este din subarborele drept
	// idx se actualizeaza fata de root->weight
	return __indexRope(root->right, idx - root->weight);
}

// Functia principala de index
char indexRope(RopeTree* rt, int idx) {
	return __indexRope(rt -> root, idx);
}

// Functia principala de search
char* search(RopeTree* rt, int start, int end) {
	...
	// Caut fiecare litera din intervalul dat si o adaug la secventa
